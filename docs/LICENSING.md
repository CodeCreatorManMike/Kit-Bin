...
| N/A — `src/lib/dev/md5.ts` | MD5 hashing for the File Hash Checker (`/dev/sha256`) | No dependency added — MD5 is a fully public, unpatented, decades-old algorithm (RFC 1321), so it's hand-implemented from the public spec directly in the codebase rather than pulled in as a package. Web Crypto's `SubtleCrypto.digest()` doesn't implement MD5 natively (only SHA-1/256/384/512), which is the actual reason a package would otherwise seem necessary. | Yes — no third-party license question applies to a from-scratch implementation of a public algorithm. |
| `@techstark/opencv-js` | Perspective correction/edge detection (`/pdf/scan-to-pdf`) | Apache-2.0, verified in `node_modules/@techstark/opencv-js/LICENSE` (OpenCV itself has been Apache-2.0 since v4.5, matching this build) | Yes. **Not imported via ESM** — its UMD build is vendored verbatim as a static asset at `public/vendor/opencv.js` and loaded with a plain `<script>` tag. A bundled `import('@techstark/opencv-js')` broke in production only: the package's factory can return either the ready module or a bare `new Promise(...)` depending on WASM-compile timing, and Astro/Vite's automatic dynamic-import preload/interop wrapping mishandled that shape (`Method Promise.prototype.then called on incompatible receiver`, reproduced on kit-bin.com, never in `astro dev`). To refresh the vendored copy after a version bump: `npm update @techstark/opencv-js && cp node_modules/@techstark/opencv-js/dist/opencv.js public/vendor/opencv.js`. |
| `kokoro-js` | Text-to-speech (`/audio/text-to-speech`) | Apache-2.0, verified in `node_modules/kokoro-js/LICENSE` | Licence is clean — model weights (`onnx-community/Kokoro-82M-v1.0-ONNX`) are separately Apache-2.0 per the model card — but the **tool is currently disabled** for an unrelated functional bug, not a licensing one. `kokoro-js` statically imports `phonemizer` (an Emscripten/WASM package); Astro/Vite's production bundling of that already-built package corrupts its voice/language data — confirmed by reproducing on kit-bin.com, then bisecting: `phonemizer`'s `list_voices()` returns 8 real voices loaded fresh from a CDN, but returns zero inside Kit-Bin's bundle, so every generation deterministically throws `Invalid language identifier: "en-us". Should be one of: .` after the ~80MB model download completes. Same *class* of bug as the opencv-js issue below, but not fixable the same way (vendoring as a static `<script>`) since `phonemizer` is imported by kokoro-js's own dist file, not by code we control — a real fix means patch-package-ing kokoro-js itself. See the doc comment at the top of `src/pages/audio/text-to-speech.astro` for the full writeup; that page currently shows a plain "temporarily offline" message and the tool is delisted from `tools.ts` (search/hub) rather than shipped broken. The library module (`src/lib/audio/textToSpeech.ts`) is untouched and ready to go the moment this is fixed. |
